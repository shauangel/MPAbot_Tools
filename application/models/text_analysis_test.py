...
class TextAnalyze:
    ...
    POS_TAG = ['NOUN', 'PROPN', 'VERB', 'ADJ', 'ADV']  # 欲留下的詞類noun
    WHITE_LIST = ['pandas', 'cors']
    ...
    def content_pre_process(self, doc):
        ...
        filtered_token = [word for word in pure_word if word.text not in self.STOPWORDS]
        ...
        for token in filtered_token:
            if token.pos_ in self.POS_TAG:
                ...
                lemma.append(temp)
                try:
                    pos_tag[token.pos_] += [temp]
                except:
                    pos_tag[token.pos_] = [temp]

        """
        lemma = [token.text if token.lemma_ == "-PRON-" or token.text in self.WHITE_LIST
                  else token.lemma_ for token in filtered_token if token.pos_ in self.POS_TAG]
        """
        # return pure_word
        return {"result": lemma, "pos_tag_list": pos_tag}
...
if __name__ == "__main__":
    ...
    test = "Showing an preprocessing example"
    nlp = spacy.load('en_core_web_sm', disable=['ner'])
    stop = nltk.corpus.stopwords.words('english')
    doc = nlp(test)
    print([t.text for t in doc])
    print([t.text for t in doc if t.text not in stop])
    stopped = [t.lemma_ for t in doc if t.text not in stop]
    print(stopped)
    print([t.pos_ for t in doc if t.text not in stop])
    stemmer = PorterStemmer()
    singles = [stemmer.stem(plural) for plural in stopped]
    print(singles)
